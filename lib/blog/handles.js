module.exports = function getEndPoints(configuration) {
 ...
 var validator = conf.authorvalidator;
    function hashUser(author, callback) {
        var query = {};
        query(validator) = author;
        Posts.find(query, function (err, reply) {
            //insertsomerandomhashofauthor
            var newreply = reply
            if (err) {
                return callback(err);
            }
            return callback(null, newreply);
        });
    }
